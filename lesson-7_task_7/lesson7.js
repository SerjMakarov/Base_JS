 ...
 */
function startGame() {
    wallTimer = setInterval(createWall, 5000); //Создает препятствия на игровом поле.
    createElScope(); //Создает элемент, в котором будет вывод счета.
    gameIsRunning = true;
...
function createWall(){
    ...
    while (!wallCreated) { //пока еду не создали
        ...
        // console.log(wall_cell_classes);

        // проверка на змейку
        if (!wall_cell_classes.includes('wall-unit')) {
            var classes = '';
...
function getScope(score){
    elDiv =  document.querySelector('.scope');
    elDiv.textContent = 'Ваш счет: ' + score;
}

function createElScope(){
    var elScore = document.createElement('div');
    var elWrap = document.querySelector('.wrap');
    elWrap.append(elScore);
    elScore.className = 'scope';
    elScore.textContent = 'Ваш счет: ';
}
